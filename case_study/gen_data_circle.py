...
def gen_negative_graph(node_idx):
    ...
    os_path = f'gen_circle_data/{random_changed_ratio}/{node_idx}'
    if not os.path.exists(os_path):
        os.makedirs(os_path)

    json_matrix = json.dumps(negative_data_dict)
    with open(f'gen_circle_data/{random_changed_ratio}/{node_idx}/negative.json',
              'w') as json_file:
        json_file.write(json_matrix)
    print('save success')

    json_matrix = json.dumps(positive_data_dict)
    with open(f'gen_circle_data/{random_changed_ratio}/{node_idx}/positive.json',
              'w') as json_file:
        json_file.write(json_matrix)
    print('save success')
...
if __name__=='__main__':
    ...
    dataset = ExplainerDataset(
        ...
        motif_generator=CycleMotif(num_nodes=5),
        num_motifs=1,
